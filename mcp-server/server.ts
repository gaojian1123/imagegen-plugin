...
export function buildServer(client: OpenAI, deployment: string): McpServer {
  ...
  const common = {
    ...
    output_format: z.enum(["png", "jpeg", "webp"]).default("png").describe("Saved image file format."),
  ...
  registerAppTool(
    ...
    (args, extra) =>
      args.partial_images > 0
        ? runStream(args, extra, (client, deployment) => editStream(client, deployment, args, store))
