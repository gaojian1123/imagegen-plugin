...
// like ext-apps' basic-host. Run: npm run dev:host  (then point basic-host here).
//
...
const httpServer = createServer(async (req, res) => {
  ...
  if ((req.url ?? "").split("?")[0] !== "/mcp") { res.writeHead(404); res.end("not found"); return; }
...
httpServer.listen(PORT, () => {
  ...
  console.log(`  SERVERS='["http://localhost:${PORT}/mcp"]' npm run start   # then open http://localhost:8080`);
});
