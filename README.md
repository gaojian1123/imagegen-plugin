...
## Develop
The server is written in TypeScript (`mcp-server/*.ts`); the shipped
`mcp-server/dist/index.js` is a self-contained esbuild bundle, and the App UI
(`mcp-server/ui/`) is bundled by Vite (`vite-plugin-singlefile`) into a single
self-contained `mcp-server/dist/mcp-app.html`. Running the tests requires Node 24
(native TypeScript execution); the bundled server itself runs on Node 18+.
```shell
npm install
npm run typecheck  # tsc --noEmit (type check)
npm test           # unit tests (no network)
npm run build      # build the UI + bundle the server into dist/
npm run smoke      # verify the built server lists its tools
```
...
```shell
npm run dev:check   # headless: proves streamed partials are served live during a
...
```shell
npm run dev:host    # serves http://localhost:3001/mcp (fake images)
