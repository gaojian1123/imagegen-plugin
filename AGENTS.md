...
- **TypeScript source vs bundle:** the server lives in `mcp-server/*.ts` — tool/
  ...
  the stdio entry in `index.ts`, and the shared Streamable HTTP adapter in
  `http.ts`. The committed `mcp-server/dist/index.js` is the tsdown bundle
  generated by `vp pack` — never edit it by hand. The App UI lives in
  ...
  committing. `vp run http` serves the real Azure backend. Dev-only harness
  (fake images, no Azure): `vp run dev:check` (headless) and `vp run dev:host`
  (Streamable HTTP for a browser host).
